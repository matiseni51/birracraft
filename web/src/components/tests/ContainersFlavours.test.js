...
test("elements in ContainersFlavours", () => {
  expect(
    screen.getByRole("heading", { name: "Containers", level: 4 }),
  ).toBeInTheDocument();
  expect(
    screen.getByRole("heading", { name: "Flavours", level: 4 }),
  ).toBeInTheDocument();
  ...
  expect(
    screen.getByRole("columnheader", { name: "Liters" }),
  ).toBeInTheDocument();
  ...
  expect(
    screen.getByRole("columnheader", { name: "Price per lt" }),
  ).toBeInTheDocument();
  ...
  expect(screen.getAllByRole("row", { name: /rows per page/i })).toHaveLength(
    2,
  );
});

test("click Container New button - elements", () => {
  const newContainersButton = screen.getAllByRole("button", {
    name: /new/i,
  })[0];
  ...
  expect(
    screen.getByRole("dialog", { name: "Create New Container" }),
  ).toBeInTheDocument();
  expect(
    screen.getByRole("heading", { name: "Create New Container", level: 2 }),
  ).toBeInTheDocument();
...
test("click Flavours New button - elements", () => {
  ...
  expect(
    screen.getByRole("dialog", { name: "Create New Flavour" }),
  ).toBeInTheDocument();
  expect(
    screen.getByRole("heading", { name: "Create New Flavour", level: 2 }),
  ).toBeInTheDocument();

  expect(screen.getAllByRole("textbox")).toHaveLength(3);
  expect(screen.getByRole("textbox", { name: "Name" })).toBeInTheDocument();
  expect(
    screen.getByRole("textbox", { name: "Price per liter" }),
  ).toBeInTheDocument();
  expect(
...
test("click Container New button - click Save success", async () => {
  ...
  expect(
    screen.getByText("Add a new container to assign to a product."),
  ).toBeInTheDocument();
...
test("click Flavour New button - click Save success", async () => {
  ...
  expect(
    screen.getByText("Add a new flavour to assign to a product."),
  ).toBeInTheDocument();
