...
test("elements in Products", () => {
  ...
  expect(
    screen.getByRole("columnheader", { name: "Flavour" }),
  ).toBeInTheDocument();
  expect(
    screen.getByRole("columnheader", { name: "Arrived Date" }),
  ).toBeInTheDocument();
  expect(
...
test("click Containers & Flavours button", async () => {
  const cfButton = screen.getByRole("button", {
    name: /containers & flavours/i,
  });
...
test("click New button - elements", () => {
  ...
  expect(
    screen.getByRole("dialog", { name: "Create New Product" }),
  ).toBeInTheDocument();
  expect(
    screen.getByRole("heading", { name: "Create New Product", level: 2 }),
  ).toBeInTheDocument();
  ...
  for (let element of document.getElementsByTagName("input")) {
    if (element.name)
      expect(element.name).toMatch(/container|flavour|code|arrived_date|state/);
  }
