...
test("elements in SignIn page", () => {
  ...
  expect(screen.getByRole("link", { name: /forgot password/i }).href).toContain(
    "/ResetPass",
  );
  expect(screen.getByRole("link", { name: /have an account/i }).href).toContain(
    "/SignUp",
  );
});

test("click Reset Password text link", () => {
  const resetPassLink = screen.getByRole("link", { name: "Forgot password?" });
...
test("click Sign Up text link", () => {
  const signUpLink = screen.getByRole("link", { name: /have an account\?/ });
  ...
  expect(screen.getByTestId("LockOutlinedIcon")).toBeInTheDocument();
  expect(
    screen.getByRole("heading", { name: "Sign up", level: 1 }),
  ).toBeInTheDocument();
...
test("submit-click Sign In button fails", async () => {
  ...
  expect(
    screen.getByRole("heading", { name: /password incorrect/i, level: 2 }),
  ).toBeInTheDocument();
  expect(
    screen.getByText("The input data is incorrect. Check the credentials used"),
  ).toBeInTheDocument();
});
