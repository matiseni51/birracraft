...
test("auth-TopBar click Payments link", async () => {
  ...
  // Title
  expect(screen.getByRole("heading", { name: "Payments", level: 3 })).toBeInTheDocument();
...
test("auth-TopBar click Profile link", async () => {
  ...
  expect(screen.getByRole("heading", { name: "Profile Data", level: 1 })).toBeInTheDocument();

  expect(screen.getByRole("textbox", {name: "sam"}).id).toContain("username");
  expect(screen.getByRole("textbox", {name: "samuel"}).id).toContain("firstName");
  expect(screen.getByRole("textbox", {name: "smith"}).id).toContain("lastName");
  expect(screen.getByRole("textbox", {name: "ss@birracraft.io"}).id).toContain("email");
